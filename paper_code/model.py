...
class GenderLSTM(nn.Module):
    ...
    def __init__(
        self, datagenerator, embedding_dim, hidden_dim, device="cpu", reversed=False
    ):
    ...
    def forward(self, inputs):
        ...
        # "lstm_out" contains the hidden states for EACH time step in the input sequence for 
        #   each element in the batch. Shape: [batch_size, sequence_length, hidden_size]
        # "hidden" is the hidden state for the LAST time step in the input sequence for each element 
        #   in the batch. Shape: [num_layers(=1) * num_directions(=1), batch_size, hidden_size]
        lstm_out, (hidden, _) = self.lstm(embeds)

        # "out_logits" will be used to get accuracy values for each time step
        out_logits = self.fc(lstm_out)  # shape: [batch_size, sequence_length, num_classes]

        # "hidden_logits" will be used for loss computation
        hidden_logits = self.fc(
            hidden[-1]  # last hidden state of the last layer
        )  # shape: [batch_size, num_classes]

        # Probability distribution over the classes for each character in the sequence 
        # (for easier interpretability)
        out_probabilities = self.softmax(
            out_logits
        )
    ...
    def train_model(
        ...
        for epoch in range(n_epochs):
            ...
            # will contain the INDEX of the position at which the accuracy starts to plateau for each batch
            train_plateau_indecies = [] 
            valid_plateau_indecies = []
            
            # will contain the ACCURACY of the position at which the accuracy starts to plateau for each batch
            train_plateau_accuracies = []   
            valid_plateau_accuracies = []

            # {token: [0/1 for incorrect/correct prediction at each character position in the token]}
            train_char_prediction_probs = {}
            valid_char_prediction_probs = {}
            ...
            for inputs, labels in traingenerator.generate_batches(batch_size):
                ...
                # Y_seq_probs: [batch_size, sequence_length, num_classes] 
                # Y_hidden_logits: [batch_size, num_classes]
                ...
                for word_idx in range(len(X)):
                    ...
                    train_char_prediction_probs[word] = [
                        (
                            traingenerator.input_idx2sym[char_id],
                            {
                                traingenerator.output_idx2sym[class_idx]: Y_seq_probs[
                                    word_idx, char_idx, class_idx
                                ].item()
                                for class_idx in range(num_classes)
                            },
                        )
                        for char_idx, char_id in enumerate(X[word_idx])
                        if char_id != pad_idx
                    ]

                # Plateau info
                plateau_beginning, plateau_confidence = self.get_plateau_info(
                    Y_seq_probs, threshold=0.1, patience=3
                )
                    ...
                    for word_idx in range(len(X)):
                        ...
                        valid_char_prediction_probs[word] = [
                            (
                                validgenerator.input_idx2sym[char_id],
                                {
                                    validgenerator.output_idx2sym[
                                        class_idx
                                    ]: Y_seq_probs[word_idx, char_idx, class_idx].item()
                                    for class_idx in range(num_classes)
                                },
                            )
                            for char_idx, char_id in enumerate(X[word_idx])
            ...
            avg_train_accuracy = sum(epoch_train_accuracies) / len(
                epoch_train_accuracies
            )
            ...
            train_avg_acc_at_mode_plateau_beg = statistics.mean(
                [
                    train_plateau_accuracies[idx].item()
                    for idx, value in enumerate(train_plateau_indecies)
                    if value == train_plateau_beg_mode
                ]
            )

            avg_valid_accuracy = sum(epoch_valid_accuracies) / len(
                epoch_valid_accuracies
            )
            ...
            valid_avg_acc_at_mode_plateau_beg = statistics.mean(
                [
                    valid_plateau_accuracies[idx].item()
                    for idx, value in enumerate(valid_plateau_indecies)
                    if value == valid_plateau_beg_mode
                ]
            )
            ...
            if verbose:
                print(
                    ...
                    f"Accuracy: {avg_train_accuracy * 100:.2f}%   "
                    f"Beginning of plateau (index): {train_plateau_beg_mode}   "
                    f"Accuracy at the beginning of plateau: "
                    f"{train_avg_acc_at_mode_plateau_beg * 100:.2f}%"
                )
                print(
                    f"[Valid] Loss: {avg_valid_loss:.4f}   "
                    f"Accuracy: {avg_valid_accuracy * 100:.2f}%   "
                    f"Beginning of plateau (index): {valid_plateau_beg_mode}   "
                    f"Accuracy at the beginning of plateau: "
                    f"{valid_avg_acc_at_mode_plateau_beg * 100:.2f}%"
                )
                print("-" * 100)
            ...
            if counter == patience:
                if verbose:
                    print(
                        f"Early stopping after {epoch + 1} epochs and {patience} epochs without improvement."
                    )
                break
    ...
    def predict(self, datagenerator, batch_size, set=str):
        ...
        for inputs, labels in datagenerator.generate_batches(batch_size):
            with torch.no_grad():
                ...
                # probs: [batch_size, sequence_length, num_classes]
                ...
                for i in range(len(X)):
                    word = "".join(
                        [
                            datagenerator.input_idx2sym[idx]
                            for idx in X[i]
                            if idx != pad_idx
                        ]
                    )
                    ...
                    predictions["Predicted Gender"].append(
                        datagenerator.output_idx2sym[Y_pred[i]]
                    )
                    predictions["True Gender"].append(
                        datagenerator.output_idx2sym[Y[i]]
                    )
                    ...
                    predictions["Class Probabilities"].append(
                        [
                            (
                                ...
                                {
                                    datagenerator.output_idx2sym[class_idx]: probs[
                                        i, char_idx, class_idx
                                    ].item()
                                    for class_idx in range(probs.size(-1))
                                },
                            )
                            for char_idx, char_id in enumerate(X[i])
                            if char_id != pad_idx
                        ]
                    )
    ...
    def get_plateau_info(self, Y_probs, threshold=0.01, patience=3):
        ...
        # Shows the average confidence in prediction for each character position 
        #   regardless of whether the prediction is correct or not
        avg_pos_confidence = prediction_probs.mean(dim=0) 

        # self.plot_confidence_curve(avg_pos_confidence.tolist())

        # If the confidence level doesn't increase by a value higher than the threshold 
        #   for <patience> consecutive characters, extract the index and accuracy of 
