...
def pad_sequence(sequence, pad_size, pad_token):
    """
    Returns a list of the characters in the sequence
    with additional pad tokens to match pad_size if needed
    """
    return list(sequence) + [pad_token] * (pad_size - len(sequence))


def encode_sequence(charseq, encodingmap, unk_token="<unk>"):
    # charseq is a sequence of characters
    return [
        encodingmap[char] if char in encodingmap else encodingmap[unk_token]
        for char in charseq
    ]
...
def save_probabilities(probabilities, df, filename, mode, set):
    ...
    assert mode in [
        "w",
        "a",
    ], "The mode needs to be either 'w' (to overwrite the file) or 'a' (to append to the file)"
...
class DataGenerator:
    ...
    def __init__(
        self,
        df,
        parentgenerator=None,
        reverse_nouns=False,
        pad_token="<pad>",
        unk_token="<unk>",
    ):

        if parentgenerator is not None:  # Reuses the encodings of the parent
    ...
    def generate_batches(self, batch_size):
        ...
        while bstart < N:
            ...
            padded_X = [
                pad_sequence(self.X[idx], batch_len, self.pad_token)
                for idx in batch_idxes
            ]
            #   save_padded_words('../data/eval/padded_nouns', padded_X)
            seqX = [
                encode_sequence(seq, self.input_sym2idx, self.unk_token)
                for seq in padded_X
            ]
